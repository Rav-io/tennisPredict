...
const EloHistory = () => {
    ...
    const fetchEloPlot = async () => {
        const response = await fetch('http://127.0.0.1:5000/get_elo_history', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            console.error('Failed to fetch Elo plot');
        }
    };
    ...
    return (
        <div>
            ...
            {eloPlot && <img src={`data:image/png;base64,${eloPlot}`} id="plot" alt="Elo History Plot" />}
        </div>
    );
};
